...
int main() {
    ...
    printf("static const uint8_t cabac_contexts[4][QP_MAX_SPEC+1][1024] = {\n");
    for (int i=0; i < 4; i++) {
        printf("    {\n");
        for (int j=0; j <= QP_MAX_SPEC; j++) {
            printf("        {\n");

            uint8_t *cabac_context = cabac_contexts[i][j];
            for (int k=0; k<1024; k++) {
                if (k % 16 == 0)
                    printf("\n           ");
                printf(" %3d,", cabac_context[k]);
            }
            printf("\n        },\n");
        }
        printf("\n    },\n");
